...
class Meta_tribunal:
    ...
    def __str__(self):
        return f"{self.sigla_tribunal,self.meta1,self.meta2a,self.meta2ant,self.meta4a,self.meta4b}\n"
...
def primeira_linha():
    with open('metas_municipio.csv', 'w', encoding='utf-8') as file: 
        file.write(f"Municipio,Meta1,Meta2a,Meta2ant,Meta4a,Meta4b\n")

def primeira_linha_tribunal():
...
def metas_resumo_municipio_paralel(linhas):
...
def resumo_tribunal_paralel(linhas):    
...
match op2:
    case "1":
        ...
        thread4 = threading.Thread(target=escrever_arquivo_inteiro_paralel,args=(lista4,))
        thread1.start()
        thread2.start()
        thread3.start()
        thread4.start()
    ...
    case "2":
        ...
        thread3 = threading.Thread(target=metas_resumo_municipio_paralel(lista3))
        thread4 = threading.Thread(target=metas_resumo_municipio_paralel(lista4))
        thread1.start()
        thread2.start()
        thread3.start()
        thread4.start()
    ...
    case "3":
        ...
        thread3 = threading.Thread(target=resumo_tribunal_paralel,args=(lista3,))
        thread4 = threading.Thread(target=resumo_tribunal_paralel,args=(lista4,))
        thread1.start()
        thread2.start()
        thread3.start()
        thread4.start()
