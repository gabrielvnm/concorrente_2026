...
class Linha:
    ...
    """
    def __str__(self):
        return f"Linha = {self.sigla_tribunal,self.procedimento, 
                            self.ramo_justica, self.sigla_grau,
...
print("1: versão serializada\n2: versão paralelizada\n")
...
match op:
    ...
    case "2":
        linhas = concatenar_arquivos()
        thread1 = threading.Thread(target=escrever_arquivo_inteiro,args=(linhas,))
        thread2 = threading.Thread(target=metas_resumo_municipio,args=(linhas,))
        thread3 = threading.Thread(target=resumo_tribunal, args=(linhas,))
        thread4 = threading.Thread(target=resumo_municipio, args=(linhas,))
        thread1.start()
        thread2.start()
        thread3.start()
        thread4.start()
